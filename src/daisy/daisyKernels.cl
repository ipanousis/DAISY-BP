                        ...
                        const    int     templateNoOffset)
{
  ...
  const int searchNo = (get_global_id(0) / WGX_MATCH_MIDDLE) * WG_TARGETS_NO;
...
#if OUTPUT <= (WGX_MATCH_MIDDLE / 32)

    if(lx < WGX_MATCH_MIDDLE / 32){
...
#if OUTPUT <= (WGX_MATCH_MIDDLE / 64)

    if(lx < WGX_MATCH_MIDDLE / 64){
