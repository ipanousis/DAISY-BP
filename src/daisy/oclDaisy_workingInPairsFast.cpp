...
int oclDaisy(daisy_params * daisy, ocl_constructs * daisyCl, time_params * times){
  ...
  unsigned long int daisySectionSize = daisyBlockWidth * daisyBlockHeight * (daisy->totalPetalsNo + TRANSD_FAST_PETAL_PADDING) * daisy->gradientsNo * sizeof(float);
  ...
  if(totalSections == 1){
    ...
    unsigned long int daisyDescriptorSize = daisy->paddedWidth * daisy->paddedHeight * (daisy->totalPetalsNo + TRANSD_FAST_PETAL_PADDING) * daisy->gradientsNo * sizeof(float);
...
if(DEBUG_ALL){
    ...
    float * daisyArray = daisy->descriptors + sectionNo * daisyBlockHeight * daisyBlockWidth * 200;
    float * transArray = (float*)malloc(paddedWidth * paddedHeight * 8 * 3 * sizeof(float));

    error = clEnqueueReadBuffer(daisyCl->queue, transBuffer, CL_TRUE,
                                0, paddedWidth * paddedHeight * 8 * 3 * sizeof(float), transArray,
                                0, NULL, NULL);

    clFinish(daisyCl->queue);
    int petalStart = 1;
    for(int r = 0; r < 64; r+=16){
      for(int c = 0; c < 16*8; c+=1){
        //for(k = petalStart * 8; k < (petalStart+4) * 8; k++)
        //  printf(", %f",daisyArray[(r * 512 + 128) * 25 * 8 + k]);
        printf(", %f",daisyArray[r*200*512+c]);
      }
      printf("\n\n");
    }
    //continue;
